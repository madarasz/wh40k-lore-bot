...
**ARCHITECTURE NOTE:** ChromaDB is now the **sole storage** for chunk data (text, embeddings, and metadata). The redundant SQLite `wiki_chunk` table was removed. ChromaDB stores both embeddings and full chunk data, serving as the single source of truth. The `WikiChunk` ORM model was replaced with a `ChunkData` TypedDict for lightweight data transfer.
...
3. Collection metadata schema:
   ...
   - `section_path`: string
   - `chunk_text`: string (stored in ChromaDB documents)
   - `chunk_index`: int
   - `metadata`: dict (contains faction, era, spoiler_flag, content_type, etc.)
4. `add_chunks(chunks: List[ChunkData], embeddings: List[np.ndarray])` method (uses ChunkData TypedDict, not ORM model)
5. Batch insertion: 1000 chunks per batch (Chroma performance)
6. `query(query_embedding: np.ndarray, n_results: int, filters: Dict) -> List[Tuple[ChunkData, float]]` method (returns chunks with similarity scores)
...
**Current Implementation** (replaces WikiChunk ORM model):
...
class ChunkData(TypedDict):
    id: str                      # Chunk UUID
    wiki_page_id: str            # Wiki page identifier
    ...
    metadata: dict[str, Any]     # Contains: faction, era, spoiler_flag, content_type, etc.
```

This TypedDict provides a lightweight data structure for chunk data without requiring SQLAlchemy ORM overhead.
...
- Pull Request: https://github.com/madarasz/wh40k-lore-bot/pull/9
- **Architecture Update (2026-01-02)**: ChromaDB promoted to sole chunk storage; redundant SQLite wiki_chunk table removed; WikiChunk ORM model replaced with lightweight ChunkData TypedDict
...
|------|---------|-------------|--------|
| 2026-01-02 | 1.2 | **Architecture Change**: ChromaDB promoted to sole chunk storage; SQLite wiki_chunk table removed; WikiChunk ORM replaced with ChunkData TypedDict | Dev Agent |
