...
6. Configuration via environment:
   ...
   - `BM25_TOKENIZE_LOWERCASE` (default: True)
...
9. BM25 index building integrated into ingestion pipeline:
   ...
   - Step-by-step pipeline includes BM25: chunk → build-bm25 → embed → store
...
- [x] Task 12: Update `poetry run ingest` to include BM25
  ...
  - [x] Tested full pipeline: `poetry run ingest` → BM25 index created successfully
...
- [x] Task 13: Optimize BM25 index storage (Completed)
  ...
  - [x] **Implemented Optimization**:
    ...
    - To:
      ...
      {
          "bm25": BM25Okapi,
          "chunk_ids": list[str],  # Ordered list to map BM25 indices → chunk IDs
...
4. Imports: stdlib → third-party → local (absolute imports only)
...
- For 10K articles, 100K chunks → ~200MB index size
...
BM25_TOKENIZE_LOWERCASE=true                     # Lowercase tokens (default: true)
```
...
- Verify persistence: save → load → search consistency
...
1. Generate chunk IDs using `generate_chunk_id(wiki_page_id, chunk_index)` → `f"{wiki_page_id}_{chunk_index}"`
...
- ✅ Tasks 1-9: BM25Repository fully implemented with all core functionality (build, search, persist, update)
- ✅ Comprehensive test coverage: 24 unit tests + 9 integration tests (33 total), all passing
- ✅ Code coverage: 92% for [bm25_repository.py](src/repositories/bm25_repository.py)
- ✅ Linting and type checking pass cleanly (ruff + mypy)
- ✅ All original acceptance criteria met
- 📝 **Technical Details:** Implemented simple whitespace+lowercase tokenization as specified. BM25Okapi algorithm from rank-bm25 library used. Pickle serialization for index persistence.

**Phase 2 - CLI Integration (Added Scope - Completed):**
- ✅ **Tasks 10-12 Completed:** BM25 indexing fully integrated into ingestion pipeline
- 🎯 **Integration Points:**
  - Task 10: ✅ Integrated into `poetry run store` command - builds BM25 index automatically
  - Task 11: ✅ Added standalone `poetry run build-bm25` command for debugging
  - Task 12: ✅ Integrated into `poetry run ingest` pipeline - builds BM25 index after vector storage
- 📋 **Updated Pipeline Flow:**
  ```
  poetry run store <embeddings.json>  → Stores vectors + builds BM25 index
  poetry run build-bm25 <chunks.json> → Standalone BM25 index building (debugging)
  poetry run ingest                   → Full pipeline (includes BM25 via IngestionPipeline)
  ```
...
- ✅ Fixed chunk ID format inconsistency
- 🔧 **Issue:** BM25 was using `{article_title}:{chunk_index}` while vector store uses `{wiki_page_id}_{chunk_index}`
- ✅ **Resolution:**
...
- ✅ **Verification:** All 493 test chunks now use correct format (e.g., `16481_0` instead of `Blood Angels:0`)

**Phase 4 - Storage Optimization (Task 13 - Completed):**
- ✅ **Optimization Implemented:** BM25 pickle file no longer stores redundant data
...
- ✅ **Validation:**
  ...
  - Linting and type checking pass cleanly (ruff + mypy)
- 📝 **Bug Fixed:** Discovered and fixed tokenization config not being saved/loaded, causing search inconsistency after load
